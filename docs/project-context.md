...
4.  **SDK Explorer**: Update `src/pages/sdk-explorer.astro` if there are breaking changes to SDK methods.
5.  **Full Project Check**: After regenerating the SDK, always run a type check (`astro check` or build) to identify any breaking changes in other `.astro` or `.ts` files that consume the SDK types.
